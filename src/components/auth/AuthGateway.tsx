  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#EBF8FE] via-white to-[#DCF3FD] text-[#0F172A] flex flex-col justify-between selection:bg-[#00D2FF] selection:text-black">
      ...
      <header className="sticky top-0 z-40 bg-white/98 backdrop-blur-md border-b border-sky-100 shadow-xs">
        <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16 sm:h-20 gap-2">
            ...
            <div className="flex items-center gap-2 sm:gap-3 shrink">
              <div onClick={onBackToWelcome} className={onBackToWelcome ? "cursor-pointer shrink" : "shrink"}>
                <PartnerPlusLogo />
              </div>
              {onBackToWelcome && (
                <button
                  ...
                  className="hidden md:inline-flex items-center gap-1 text-xs font-bold text-[#1D68ED] hover:bg-blue-100/60 bg-blue-50 px-3 py-1.5 rounded-full border border-blue-200 cursor-pointer ml-1 whitespace-nowrap shrink-0 transition"
                >
            ...
            {/* Right: Actions, Language & Navigation Controls (Desktop sm: and up) */}
            <div className="hidden sm:flex items-center gap-2.5 sm:gap-3 shrink-0 whitespace-nowrap">
              {/* Phone Helpline */}
              <a 
                href="tel:2145505563" 
                className="hidden lg:inline-flex items-center gap-1.5 text-xs font-extrabold text-gray-900 hover:text-[#1D68ED] transition bg-gray-50 px-3 py-1.5 rounded-full border border-gray-200/80 shrink-0"
              >
              ...
              {/* Language Selector */}
              <div className="flex items-center gap-1 bg-gray-50 py-1.5 px-2.5 rounded-full border border-gray-200 text-xs shrink-0">
              ...
              <button
                ...
                className={`text-xs font-bold transition px-3 py-1.5 rounded-full cursor-pointer shrink-0 flex items-center gap-1.5 ${
                  authMode === 'login' && selectedRole === 'worker'
                    ? 'text-emerald-700 bg-emerald-50 ring-1 ring-emerald-500/30'
              ...
              <button
                ...
                className={`text-xs font-extrabold transition px-3.5 py-1.5 rounded-full cursor-pointer shrink-0 border ${
                  authMode === 'signup' && signupSubView === 'selector'
                    ? 'text-[#1D68ED] bg-blue-50 border-blue-300 ring-1 ring-[#1D68ED]/30'
                    : 'text-[#1D68ED] bg-blue-50/60 border-blue-200 hover:bg-blue-100/60'
                }`}
              >
                Register
              </button>
              ...
              <button
                ...
                className="px-3.5 sm:px-4 py-1.5 sm:py-2 bg-[#00D2FF] hover:bg-[#33EBFF] text-black font-extrabold rounded-full text-xs tracking-tight shadow-xs hover:shadow-md transition-all duration-200 active:scale-98 cursor-pointer flex items-center gap-1.5 shrink-0"
              >
            ...
            {/* Right: Actions for Mobile Screens (< sm) */}
            <div className="flex items-center gap-1.5 sm:hidden shrink-0">
              <button
                type="button"
                onClick={() => { setAuthMode('signup'); setSignupSubView('worker'); }}
                className="px-2.5 py-1.5 bg-[#00D2FF] active:bg-[#33EBFF] text-black font-extrabold rounded-full text-[11px] tracking-tight flex items-center gap-1 shadow-xs shrink-0 cursor-pointer"
              >
              ...
              <button
                ...
                className="p-1.5 text-gray-700 hover:text-black hover:bg-gray-100 rounded-lg transition cursor-pointer shrink-0"
        ...
        {/* Mobile Menu Drawer */}
        {mobileMenuOpen && (
          <div className="sm:hidden border-t border-sky-100 bg-white shadow-xl px-4 pt-3 pb-5 space-y-3 animate-in slide-in-from-top-2 duration-200">
            <div className="flex items-center justify-between py-1 border-b border-gray-100 pb-2">
              ...
              <div className="flex items-center gap-1 bg-gray-50 py-1 px-2.5 rounded-full border border-gray-200 text-xs">
                ...
                >
                  ...
                  <option value="mr">மராठी (Marathi)</option>
                </select>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 gap-2 pt-1">
              <button
                type="button"
                onClick={() => { setMobileMenuOpen(false); setAuthMode('login'); setSelectedRole('customer'); handleRoleChange('customer'); }}
                className="w-full text-left px-3.5 py-2.5 rounded-xl text-xs font-bold text-gray-800 hover:bg-blue-50 hover:text-[#1D68ED] flex items-center justify-between border border-gray-100"
              >
                <span>Commercial Client Login</span>
                <ArrowRight className="w-3.5 h-3.5 text-gray-400" />
              </button>
              <button
                type="button"
                onClick={() => { setMobileMenuOpen(false); setAuthMode('login'); setSelectedRole('worker'); handleRoleChange('worker'); }}
                className="w-full text-left px-3.5 py-2.5 rounded-xl text-xs font-bold text-emerald-800 bg-emerald-50/50 hover:bg-emerald-100/60 flex items-center justify-between border border-emerald-200"
              >
                <div className="flex items-center gap-2">
                  <HardHat className="w-4 h-4 text-emerald-700" />
                  <span>Worker / Artisan Login</span>
                </div>
                <ArrowRight className="w-3.5 h-3.5 text-emerald-600" />
              </button>
              <button
                type="button"
                onClick={() => { setMobileMenuOpen(false); setAuthMode('signup'); setSignupSubView('selector'); }}
                className="w-full text-left px-3.5 py-2.5 rounded-xl text-xs font-extrabold text-[#1D68ED] bg-blue-50/70 hover:bg-blue-100/60 flex items-center justify-between border border-blue-200"
              >
                <span>Register New Account</span>
                <ArrowRight className="w-3.5 h-3.5 text-[#1D68ED]" />
              </button>
              <button
                type="button"
                onClick={() => { setMobileMenuOpen(false); setAuthMode('signup'); setSignupSubView('worker'); }}
                className="w-full py-2.5 bg-[#00D2FF] text-black font-black rounded-xl text-xs text-center flex items-center justify-center gap-1.5 shadow-xs"
              >
                <UserPlus className="w-4 h-4" />
                <span>Join As Worker / Artisan</span>
              </button>
              {onBackToWelcome && (
                <button
                  type="button"
                  onClick={() => { setMobileMenuOpen(false); onBackToWelcome(); }}
                  className="w-full py-2 bg-gray-100 hover:bg-gray-200 text-gray-700 font-bold rounded-xl text-xs text-center transition"
                >
                  ← Back to Home
                </button>
              )}
